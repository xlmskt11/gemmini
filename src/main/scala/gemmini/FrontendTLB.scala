...
class FrontendTLB(nClients: Int, entries: Int, maxSize: Int, use_tlb_register_filter: Boolean, use_firesim_simulation_counters: Boolean, use_shared_tlb: Boolean)
  ...
  val counted_tlb_reqs = Wire(Vec(nClients, Bool()))
  val counted_tlb_hits = Wire(Vec(nClients, Bool()))
  ...
  io.clients.zipWithIndex.foreach { case (client, i) =>
    ...
    val last_counted_tlb_req_valid = RegInit(false.B)
    val last_counted_tlb_req = Reg(new DecoupledTLBReq(lgMaxSize))
    ...
    val same_as_last_counted_tlb_req =
      if (use_tlb_register_filter) last_counted_tlb_req_valid && (tlbReq.asUInt === last_counted_tlb_req.asUInt) else false.B
    val counted_tlb_req = tlbReqFire && !same_as_last_counted_tlb_req

    counted_tlb_reqs(i) := counted_tlb_req
    counted_tlb_hits(i) := counted_tlb_req && !tlb.io.resp.miss

    when (tlbReqFire) {
      last_counted_tlb_req_valid := true.B
      last_counted_tlb_req := tlbReq
    }
    ...
    when (tlb.io.exp.flush()) {
      ...
      last_counted_tlb_req_valid := false.B
    }
  ...
  val dma_tlb_total_req = counted_tlb_reqs.asUInt.orR
  val dma_tlb_hit_req = counted_tlb_hits.asUInt.orR

  tlbs.foreach(_.io.counter.external_reset := false.B)

  CounterEventIO.init(io.counter)
  io.counter.connectEventSignal(CounterEvent.DMA_TLB_HIT_REQ, dma_tlb_hit_req)
  io.counter.connectEventSignal(CounterEvent.DMA_TLB_TOTAL_REQ, dma_tlb_total_req)
  io.counter.connectEventSignal(CounterEvent.DMA_TLB_MISS_CYCLE, tlbs.head.io.resp.miss)

  if (use_firesim_simulation_counters) {
    PerfCounter(dma_tlb_hit_req, "tlb_hits", "total number of non-retry tlb hits")
    PerfCounter(dma_tlb_total_req, "tlb_reqs", "total number of non-retry tlb reqs")
  }
}
